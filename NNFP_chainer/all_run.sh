...
python input_attention_one_hot_remove/chainer_regression_ecfp.py --input_file cep.csv --epochs $EPOCHS > $SAVEDIR/input_attention_ecfp_cep_remove_top_$i.txt
echo Exit ecfp  `date '+%y/%m/%d %H:%M:%S'`
python input_attention_one_hot_remove/chainer_regression_fcfp.py --input_file cep.csv --epochs $EPOCHS > $SAVEDIR/input_attention_fcfp_cep_remove_top_$i.txt
